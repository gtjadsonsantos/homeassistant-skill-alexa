...
router.get("/auth/authorize", (ctx) => {
    ...
    ctx.response.redirect(`https://beocastelo.unicontrol.me/auth/authorize?client_id=${client_id}&redirect_uri=${redirect_url}&state=${state}&response_type=${response_type}&scope=${scope}`);
});
